...
rather than at container start-time, you can run the `/scripts/jetty/gen_start_command.sh` script to generate it 
for you, i.e. 

```bash
RUN export JETTY_START_COMMAND=$(/scripts/jetty/gen_start_command.sh)
```
